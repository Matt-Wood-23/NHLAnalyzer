...
def isolate_from_real_data(tmp_path, monkeypatch):
    ...
    ``attach_outcomes`` falls back to reading ``data/parquet/feature_matrix.parquet``
    when no outcomes are passed.  On a developer machine that file exists and
    holds real games, so fixtures using real-looking game IDs silently picked up
    real results — tests that passed on a clean checkout failed on the machine
    that actually has the data.  Point the module at an empty directory so the
    fallback can never reach it.
    """
    from pipeline import evaluate_history

    sandbox = tmp_path / "sandbox"
    monkeypatch.setattr(evaluate_history, "PARQUET_DIR", sandbox / "parquet")
    monkeypatch.setattr(evaluate_history, "HISTORY_DIR", sandbox / "predictions")
    monkeypatch.setattr(
        evaluate_history, "HISTORY_PATH",
        sandbox / "predictions" / "prediction_history.parquet",
    )
